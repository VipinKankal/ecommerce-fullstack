...
const OrderItem = ({ title, size, image, orderStatus, statusDate, orderId, onOpen, id }: AccountOrderItem) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col md:flex-row justify-between">
        <div>
          <p className="text-sm font-bold text-[#282c3f] leading-tight mb-1">{title}</p>
          <p className="text-xs text-gray-500">Size: {size || 'Onesize'}</p>
          <p className="text-[10px] text-gray-400 mt-3 font-bold uppercase tracking-tighter">Order ID: #{orderId}</p>
        </div>

        <div className="mt-3 md:mt-0 md:text-right">
          <div className="flex items-center md:justify-end gap-2">
            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: statusColor }}></span>
            <span className="text-xs font-bold uppercase" style={{ color: statusColor }}>{prettyStatus(status)}</span>
          </div>
          {statusDate && <p className="text-[11px] text-gray-500 mt-1">on {statusDate}</p>}
