...
const Orders = () => {
  ...
  const [statusFilter, setStatusFilter] = useState<
    'ALL' | 'DELIVERED' | 'CANCELLED' | 'IN_PROGRESS'
  >('ALL');

  const orders = useMemo(
    () => (responses.userOrderHistory || []) as OrderLite[],
    [responses.userOrderHistory],
  );
          ...
          orderItems: (order.orderItems || []).filter((item) => {
            ...
            const statusMatch =
              ...
              (statusFilter === 'IN_PROGRESS' &&
                isInProgressStatus(displayStatus)) ||
    ...
    <div className="w-full space-y-4">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Live tracking updates now follow shipment and courier task milestones.
        </p>
      </div>
