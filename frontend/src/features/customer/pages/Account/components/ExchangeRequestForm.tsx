...
const ExchangeRequestForm = ({
  ...
  const defaultProductId = useMemo(
    () => (productOptions[0] ? String(productOptions[0].id) : ''),
    [productOptions],
  );
  const [exchangeReason, setExchangeReason] = useState('');
  const [comment, setComment] = useState('');
  const [requestedVariant, setRequestedVariant] = useState('');
  const [requestedNewProductId, setRequestedNewProductId] =
    useState<string>('');
  ...
  const resetForm = () => {
    ...
    setComment('');
    setRequestedVariant('');
    setRequestedNewProductId(defaultProductId);
    setProductPhotoName('');
  };
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <Stack spacing={2.5} sx={{ pt: 1 }}>
          ...
          <FormControl fullWidth>
            <InputLabel id="requested-new-product">
              Requested new product
            </InputLabel>
            <Select
              labelId="requested-new-product"
              value={requestedNewProductId}
              label="Requested new product"
              onChange={(e) => setRequestedNewProductId(String(e.target.value))}
            >
      ...
      <DialogActions sx={{ px: 3, pb: 3 }}>
        ...
        <Button
          variant="contained"
          disabled={!exchangeReason.trim() || !requestedNewProductId || loading}
          onClick={() =>
            onSubmit({
              exchangeReason: exchangeReason.trim(),
              comment: comment.trim() || undefined,
              productPhoto: productPhotoName || undefined,
              requestedVariant: requestedVariant.trim() || undefined,
              requestedNewProductId,
            })
          }
        >
          {' '}
