...
const AdminManualUpiPayments = () => {
  ...
  const rows = Array.isArray(responses.manualUpiPaymentsList) ? responses.manualUpiPaymentsList : [];
  ...
  const pendingCount = useMemo(
    () => rows.filter((row: any) => row.status === "PENDING_VERIFICATION").length,
    [rows],
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <Typography variant="body2" color="text.secondary">
            Review customer-submitted UPI transactions and verify or reject them.
          </Typography>
        </div>
      ...
      <TableContainer
        component={Paper}
        sx={{ borderRadius: "24px", boxShadow: "none", border: "1px solid #eef2f7" }}
      >
      ...
      <Dialog open={Boolean(selected)} onClose={() => setSelected(null)} fullWidth maxWidth="sm">
        ...
        <DialogContent className="space-y-4">
          {selected && (
            <div className="space-y-3 pt-2">
              ...
              <div>
                <Typography variant="body2">Transaction ID: {selected.transactionId || "N/A"}</Typography>
                <Typography variant="body2">Payment App: {selected.paymentApp || "N/A"}</Typography>
                <Typography variant="body2">Amount: Rs {selected.amount || 0}</Typography>
              </div>
        ...
        <DialogActions>
          ...
          <Button color="error" variant="outlined" onClick={() => handleVerify("REJECTED")}>
            Reject
          </Button>
