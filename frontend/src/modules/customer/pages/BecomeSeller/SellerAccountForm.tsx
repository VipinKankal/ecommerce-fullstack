...
import { Button, Step, StepLabel, Stepper, Box, Alert, CircularProgress } from '@mui/material';
...
import { registerSeller } from '../../../State/Seller/SellerAuthThunks';

const steps = ['Personal Info', 'Business Info', 'Address', 'Bank Details', 'KYC', 'Store Info'];
...
const validationSchemas = [
  ...
  Yup.object({
    gstin: Yup.string().length(15, 'GSTIN must be 15 chars').required('Required'),
  ...
  Yup.object({
    storeDetails: Yup.object({
      ...
      supportPhone: Yup.string().matches(/^[0-9]{10}$/, 'Must be 10 digits').required('Required'),
    }),
  }),
];
...
const SellerAccountForm = ({ onRegisterSuccess }: SellerAccountFormProps) => {
  ...
  const handleNext = async () => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      formik.setTouched(
        Object.keys(errors).reduce((acc, key) => ({ ...acc, [key]: true }), {} as any),
      );
    }
  };
