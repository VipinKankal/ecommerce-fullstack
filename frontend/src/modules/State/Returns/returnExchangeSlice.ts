...
const normalizeRequests = (payload: any): ReturnExchangeRequestRecord[] => {
...
>("returnExchange/fetchRequests", async (_, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(getErrorMessage(error, "Failed to load return and exchange requests"));
  }
});
...
>("returnExchange/submitRequest", async (payload, { rejectWithValue }) => {
  try {
    ...
    const response = await api.post(API_ROUTES.orders.returnExchange.byItem(payload.orderItemId), {
    ...
    return rejectWithValue(getErrorMessage(error, `Failed to submit ${payload.requestType.toLowerCase()} request`));
  }
});
...
const mergeRequest = (
  ...
  const existingIndex = requests.findIndex((entry) => Number(entry.id) === nextId);
  if (existingIndex === -1) {
...
const returnExchangeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchReturnExchangeRequests.fulfilled, (state, action: PayloadAction<ReturnExchangeRequestRecord[]>) => {
        state.loading = false;
        state.requests = action.payload;
      })
      .addCase(fetchReturnExchangeRequests.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || "Failed to load return and exchange requests";
      })
      ...
      .addCase(submitReturnExchangeRequest.fulfilled, (state, action: PayloadAction<ReturnExchangeRequestRecord>) => {
...
export const { clearReturnExchangeError, clearReturnExchangeSuccess } = returnExchangeSlice.actions;
